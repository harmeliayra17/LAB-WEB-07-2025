...
@section('content')
    <h1 class="text-3xl font-bold mb-4">Tambah Ikan Baru</h1>
    ...
    <form action="{{ route('fishes.store') }}" method="POST" class="max-w-lg bg-white p-6 rounded-lg shadow-md">
        @csrf
        <div class="mb-4">
            <label for="name" class="block text-sm font-medium text-gray-700">Nama Ikan</label>
            <input type="text" name="name" id="name" value="{{ old('name') }}" required class="mt-1 p-2 w-full border rounded">
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="number" step="0.01" name="base_weight_min" id="base_weight_min" value="{{ old('base_weight_min') }}" required class="mt-1 p-2 w-full border rounded">
        </div>

        <div class="mb-4">
            <label for="base_weight_max" class="block text-sm font-medium text-gray-700">Berat Maksimum (kg)</label>
            <input type="number" step="0.01" name="base_weight_max" id="base_weight_max" value="{{ old('base_weight_max') }}" required class="mt-1 p-2 w-full border rounded">
        </div>

        <div class="mb-4">
            <label for="sell_price_per_kg" class="block text-sm font-medium text-gray-700">Harga Jual per kg (Coins)</label>
            <input type="number" name="sell_price_per_kg" id="sell_price_per_kg" value="{{ old('sell_price_per_kg') }}" required class="mt-1 p-2 w-full border rounded">
        </div>

        <div class="mb-4">
            <label for="catch_probability" class="block text-sm font-medium text-gray-700">Persentase Peluang Tertangkap (%)</label>
            <input type="number" step="0.01" name="catch_probability" id="catch_probability" value="{{ old('catch_probability') }}" required class="mt-1 p-2 w-full border rounded">
        </div>
