...
@section('content')
    <div class="max-w-2xl mx-auto bg-white shadow overflow-hidden sm:rounded-lg p-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-6 border-b pb-4">Edit Ikan: {{ $fish->name }}</h1>
        ...
        <form method="POST" action="{{ route('fishes.update', $fish) }}">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="name" id="name" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 @error('name') border-red-500 @enderror" value="{{ old('name', $fish->name) }}">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" step="0.01" name="base_weight_min" id="base_weight_min" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 @error('base_weight_min') border-red-500 @enderror" value="{{ old('base_weight_min', $fish->base_weight_min) }}">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="base_weight_max" id="base_weight_max" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 @error('base_weight_max') border-red-500 @enderror" value="{{ old('base_weight_max', $fish->base_weight_max) }}">
                ...
                <div>
                    ...
                    <input type="number" name="sell_price_per_kg" id="sell_price_per_kg" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 @error('sell_price_per_kg') border-red-500 @enderror" value="{{ old('sell_price_per_kg', $fish->sell_price_per_kg) }}">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="catch_probability" id="catch_probability" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 @error('catch_probability') border-red-500 @enderror" value="{{ old('catch_probability', $fish->catch_probability) }}">
                    @error('catch_probability') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="description" class="block text-sm font-medium text-gray-700">Deskripsi</label>
                    <textarea name="description" id="description" rows="3" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2">{{ old('description', $fish->description) }}</textarea>
                </div>
            </div>
