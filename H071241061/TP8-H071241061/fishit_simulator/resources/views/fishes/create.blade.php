...
<div class="max-w-4xl mx-auto">
    ...
    <div class="glass-card rounded-2xl p-6 mb-8 shadow-ocean border border-white/10">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h2 class="text-3xl font-bold gradient-text">Tambah Ikan Baru</h2>
                <p class="text-cyan-200 text-sm">Tambahkan ikan baru ke database Fish It</p>
            </div>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('fishes.store') }}" 
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" rows="4" placeholder="Masukkan deskripsi ikan..."
                          class="w-full px-4 py-3 rounded-lg bg-white/10 border border-white/20 text-white focus:ring-2 focus:ring-cyan-400 focus:outline-none transition-all @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="flex gap-4 mt-10">
            <button type="submit" 
                    class="flex-1 py-3 bg-gradient-to-r from-cyan-400 to-blue-500 text-white rounded-lg hover:shadow-glow transform hover:scale-105 transition-all font-bold text-lg">
                💾 Simpan Ikan
            </button>
    ...
    <!-- Info Box -->
    <div class="glass-card border-l-4 border-cyan-400 rounded-xl p-5 mt-8 shadow-glow text-cyan-100">
        <div class="flex items-start space-x-3">
            <span class="text-2xl">💡</span>
            <div>
                <h4 class="font-bold text-cyan-200 mb-1">Tips Pengisian Data</h4>
                <ul class="text-sm text-cyan-300 space-y-1">
                    <li>• Pastikan berat maksimum lebih besar dari berat minimum</li>
                    <li>• Peluang tangkap semakin kecil untuk rarity yang lebih tinggi</li>
                    <li>• Harga jual biasanya berbanding lurus dengan rarity</li>
                    <li>• Deskripsi membantu pemain memahami karakteristik ikan</li>
                </ul>
            </div>
        </div>
    </div>
